...
public class PathFinding {
	...
	//UTIL ARRAYS
	private String[] algorithms = {"Dijkstra","A*","Best-First", "Uniform Cost"};
	...
	class Algorithm {	//ALGORITHM CLASS
		...
		public void UniformCostSearch() {
		    ...
		    Node finishNode = map[finishx][finishy];
		    ...
		    while (!queue.isEmpty()) {
		        ...
		        if (current == finishNode) {
		            length = current.getHops();
		            break;
		        }

		        ArrayList<Node> neighbors = getNeighbors(current);
		        for (Node neighbor : neighbors) {
		            if (!neighbor.isVisited() && neighbor.getType() != 2) {
		                neighbor.setVisited(true);
		                neighbor.setParent(current);
		                neighbor.setHops(current.getHops() + 1);
		                neighbor.setCost(current.getCost() + neighbor.getWeight());
		                queue.add(neighbor);
		            }
		        }
		    }
		    ...
		    if (length == 0) {
		        JOptionPane.showMessageDialog(frame, "Não foi possível encontrar um caminho!", "Aviso",
	...
	class Node {
		...
		public void setParent(PathFinding.Node current) {
			// TODO Auto-generated method stub
	...
	public ArrayList<PathFinding.Node> getNeighbors(PathFinding.Node current) {
		// TODO Auto-generated method stub
		return null;
	}
}